...
void BuildTriggerName( OBJECTTYPE *pItem, STR16 szItemName )
{
	...
	{
		if( pItem->bombs.bFrequency == PANIC_FREQUENCY )
			...
			swprintf( szItemName, L"Trigger%d", pItem->bombs.bFrequency - 50 );
	}
	else
	{ //action item
		if( pItem->bombs.bDetonatorType == BOMB_PRESSURE )
			...
			swprintf( szItemName, L"Action%d", pItem->bombs.bFrequency - 50 );
	}
}
