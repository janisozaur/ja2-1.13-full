...
BOOLEAN EvaluateWorld( STR8 pSector, UINT8 ubLevel )
{
	...
	for ( cnt = 0; cnt < WORLD_MAX; cnt++ )
  {
	...
	if( uiFlags & MAP_WORLDITEMS_SAVED )
	{
		...
		//the size of WORLDITEM has changed at 5.27, 26 or earlier are outdated
		if (oldMajorMapVersion >= 6.0 && oldMinorMapVersion > 26) {
			//Skip the contents of the world items.
			WORLDITEM dummyItem;
			for (int x = 0; x < pSummary->usNumItems; ++x) {
				dummyItem.Load(&pBuffer, oldMajorMapVersion, oldMinorMapVersion);
			}
		}
		else {
			//Skip the contents of the world items.
	...
	if( uiFlags & MAP_FULLSOLDIER_SAVED )
	{
		...
		for( i=0; i < pSummary->MapInfo.ubNumIndividuals ; i++ )
		{
			...
			{ //skip static priority placement 
				priority.Load(&pBuffer, oldMajorMapVersion, oldMinorMapVersion);
