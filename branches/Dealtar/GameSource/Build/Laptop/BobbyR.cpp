...
{
	LAPTOP_MODE_BOBBY_R_USED, LAPTOP_MODE_BOBBY_R_MISC, LAPTOP_MODE_BOBBY_R_GUNS, LAPTOP_MODE_BOBBY_R_AMMO, LAPTOP_MODE_BOBBY_R_ARMOR
};
...
extern MOUSE_REGION *gpSelectedDropDownRegion;
extern MOUSE_REGION *gpSelectedScrollAreaDropDownRegion;
...
BOOLEAN EnterBobbyR()
{
	...
	if (!gfBobbyRInitialized)
	{
		...
		gpSelectedDropDownRegion = new MOUSE_REGION[gPostalService.LookupDestinationList().size()];
		gpSelectedScrollAreaDropDownRegion = new MOUSE_REGION[gPostalService.LookupDestinationList().size()];
...
BOOLEAN InitBobbiesMouseRegion(UINT8 ubNumerRegions, UINT16 *usMouseRegionPosArray, MOUSE_REGION *MouseRegion)
{
	...
	for (i = 0; i < ubNumerRegions; i++)
	{
		...
		MSYS_DefineRegion(&MouseRegion[i], usMouseRegionPosArray[ubCount], usMouseRegionPosArray[ubCount + 1], usMouseRegionPosArray[ubCount + 2], usMouseRegionPosArray[ubCount + 3], MSYS_PRIORITY_HIGH, CURSOR_WWW, MSYS_NO_CALLBACK, SelectBobbiesSignMenuRegionCallBack);
