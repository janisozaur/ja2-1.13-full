...
BOOLEAN AddGameEvent( UINT32 uiEvent, UINT16 usDelay, PTR pEventData )
{
	if (usDelay == DEMAND_EVENT_DELAY)
	{
		...
		//ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"AddGameEvent: Sending Local and network #%d", uiEvent );
		#ifdef NETWORKED
		if(gfAmINetworked)
...
BOOLEAN ExecuteGameEvent( EVENT *pEvent )
{
	...
	//ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"ExecuteGameEvent: %d", pEvent->uiEvent );
	//ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"ExecuteGameEvent: %d", pEvent->pData );
	//ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"ExecuteGameEvent: %d", pEvent->uiDataSize );
...
//				DebugMsg( TOPIC_JA2, DBG_LEVEL_3, String( "Event Pump: SetPosition ( %f %f ) ( %d )", SSetPosition.dNewXPos, SSetPosition.dNewYPos, SSetPosition.usSoldierID ) );
			...
			case S_BEGINFIREWEAPON:
				...
				// Call soldier function
				//ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"Event Pump: Begin Fire Weapon");
			...
			case S_FIREWEAPON:
				...
				// Call soldier function
				//ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"Event Pump: FireWeapon");
