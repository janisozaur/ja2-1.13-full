...
// Dealtar: scripting system
#include "ScriptHost.h"
extern Ja2::Scripting::ScriptHost* gpScriptHostObj;
...
*/
		...
		{
			gpScriptHostObj->ReportGameEvent(Ja2::Scripting::ON_TRACKED_SECTOR_PLAYER_CONTROLLED, (UINT32)SECTOR(sMapX, sMapY));
			// and it's a town
			if ((bTownId >= FIRST_TOWN) && (bTownId < NUM_TOWNS))
			{
...
BOOLEAN SetThisSectorAsEnemyControlled( INT16 sMapX, INT16 sMapY, INT8 bMapZ, BOOLEAN fContested )
{
	...
	if( bMapZ == 0 )
	{
		...
		if ( fWasPlayerControlled )
		{
			...
			gpScriptHostObj->ReportGameEvent(Ja2::Scripting::ON_TRACKED_SECTOR_ENEMY_CONTROLLED, (UINT32)SECTOR(sMapX, sMapY));
