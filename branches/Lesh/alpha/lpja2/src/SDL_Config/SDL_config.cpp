...
int CFG_Internal_StringEquals(const char * s1, const char * s2, int n)
 {
  if (n == 0)
   return 0;
 ...
 */
 int CFG_Internal_IsBool(const CFG_String & value)
   {
   /* First compare sizes of bools and value, to avoid potentially slow char-by-char compare */       

    int bools_sizes[6] = {sizeof(CFG_KEYWORD_TRUE_1),  sizeof(CFG_KEYWORD_TRUE_2),
                          sizeof(CFG_KEYWORD_TRUE_3),  sizeof(CFG_KEYWORD_FALSE_1),
                          sizeof(CFG_KEYWORD_FALSE_2), sizeof(CFG_KEYWORD_FALSE_3)};
...
int CFG_OpenFile( CFG_String_Arg filename, CFG_File * file )
 {
  if (filename == 0)
   {
    ...
    /*
    commented out old library behaviour
  ...
  int result = CFG_Internal_OpenFile(src, file, filename);
  SDL_RWclose(src);
...
int CFG_Internal_FindFirst(const CFG_String & _where, CFG_Char _what, int _startPos, int _endPos)
 {
  bool insideString = false;
  bool lastWasEscapeSeq = false;
  int size = min(_where.size(), static_cast<string::size_type>(_endPos));

  for (int i = _startPos; i < size; ++i)
   {
...
CFG_Bool CFG_ReadBool(CFG_String_Arg key, CFG_Bool defaultVal)
 {
  ...
  if (defaultVal)
   entry_body.value.push_back(CFG_KEYWORD_TRUE_1);
  else
   entry_body.value.push_back(CFG_KEYWORD_FALSE_1);
...
float CFG_ReadFloat(CFG_String_Arg key, float defaultVal)
 {
  ...
  if (CFG_ENTRY_ITERATION == key)                                                                           
   {                                                                                                        
    CFG_Internal_Group * selected_group = CFG_Internal_selected_file->internal_file->selected_group;        
    if ( selected_group->multi_value_iterator != selected_group->entries.end() )                            
      return static_cast<float>(atof( (*selected_group->multi_value_iterator->second.value_iterator).c_str() ));
    else                                                                                                    
     return 0;                                                                                              
   }                                                                                                    
  ...
  if ( result == CFG_INTERNAL_FOUND )
   return ( static_cast<float>(atof( (*iterator).second.value[0].c_str() )));
...
int CFG_WriteBool(CFG_String_Arg key, CFG_Bool value)
 {
  ...
  if (CFG_ENTRY_ITERATION == key)
   {
    ...
    if ( selected_group->multi_value_iterator != selected_group->entries.end() )
     {      
      if (value)
       *selected_group->multi_value_iterator->second.value_iterator = CFG_KEYWORD_TRUE_1;
      else
       *selected_group->multi_value_iterator->second.value_iterator = CFG_KEYWORD_FALSE_1;
  ...
  if (result == CFG_OK)
   {
    if (value)
     (*iterator).second.value[0] = CFG_KEYWORD_TRUE_1;
    else
     (*iterator).second.value[0] = CFG_KEYWORD_FALSE_1;
