...
void			GetDebugLocationString( UINT16 usProfileID, STR16 pzText, UINT16 usMaxLen );
...
//	ButtonList[ iSummaryButton[ SUMMARY_PROGRESSCHECKBOX ] ]->uiFlags |= BUTTON_CLICKED_ON;
	...
	// NPC log button 
	WSTR_SPrintf( zName, 128, L"%ls - (%ls)", QuestDebugText[ QUEST_DBS_NPC_LOG_BUTTON ], gfNpcLogButton ? L"On" : L"Off");
	...
	if( giHaveSelectedNPC != -1 )
	{
		...
		if( gfUseLocalNPCs )
			WSTR_SPrintf( zItemDesc, SIZE_ITEM_INFO, L"%d - %ls", gubCurrentNpcInSector[ giHaveSelectedNPC ],  gMercProfiles[ gubCurrentNpcInSector[ giHaveSelectedNPC ] ].zNickname );
		else
			WSTR_SPrintf( zItemDesc, SIZE_ITEM_INFO, L"%d - %ls", giHaveSelectedNPC,  gMercProfiles[ giHaveSelectedNPC ].zNickname );
	...
	if( giHaveSelectedItem != -1 )
	{
		...
		WSTR_SPrintf( zItemDesc, SIZE_ITEM_INFO, L"%d - %ls", giHaveSelectedItem, zItemName );
...
//	if( !gfTextEntryActive )
	...
	if( gfAddKeyNextPass )
	{
		WSTR_SPrintf( zTemp, 512, L"  Please enter the Keys ID. ( 0 - %d )", NUM_KEYS );
...
void DisplayCurrentGridNo()
{
	if( gsQdsEnteringGridNo != 0 )
	{
		...
		WSTR_SPrintf( zTemp, 512, L"%ls:  %d", QuestDebugText[ QUEST_DBS_CURRENT_GRIDNO ], gsQdsEnteringGridNo );
				...
				case 'd':
					{
						...
						WSTR_SPrintf( zTemp, 512, L"Items dropped will be in %ls condition", gfDropDamagedItems ? L"DAMAGED" : L"PERFECT" );
...
void DisplayQuestList()
{
	...
	{
		//Display Quest Number text
		WSTR_SPrintf( sTemp, 15, L"%02d", usLoop1 );
...
void DisplayFactList()
{
	...
	{
		//Display Quest Number text
		WSTR_SPrintf( sTemp, 512, L"%02d", usLoop1 );
		...
		{			
			WSTR_SPrintf( sTemp, 512, L"No Fact %03d Yet", usLoop1 );
		...
		else
		{
			...
			{
				ReduceStringLength( sTemp, 512, QUEST_DBS_SECOND_TITLE_COL_WIDTH, QUEST_DBS_FONT_DYNAMIC_TEXT );
			}
...
//			DisplayWrappedString( QUEST_DBS_SECOND_COL_TITLE_X, usPosY, QUEST_DBS_SECOND_TITLE_COL_WIDTH, 2, QUEST_DBS_FONT_DYNAMIC_TEXT, QUEST_DBS_COLOR_STATIC_TEXT, FactDescText[ usLoop1 ], FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED	);
		...
		DrawTextToScreen( gubFact[ usLoop1 ] ? (STR16)L"True" : (STR16)L"False", QUEST_DBS_SECOND_COL_STATUS_X, usPosY, QUEST_DBS_STATUS_COL_WIDTH, QUEST_DBS_FONT_DYNAMIC_TEXT, QUEST_DBS_COLOR_DYNAMIC_TEXT, FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED	);			
...
void DisplaySelectedNPC()
{
	...
	for( i=gpActiveListBox->usItemDisplayedOnTopOfList; i< gpActiveListBox->usItemDisplayedOnTopOfList+gpActiveListBox->usNumDisplayedItems; i++)
	{
		if( gfUseLocalNPCs )
		{
			...
			GetDebugLocationString( gubCurrentNpcInSector[ i ], sTempString, 64 );
		...
		else
		{
			...
			GetDebugLocationString( i, sTempString, 64 );
...
//			GetShortSectorString( gMercProfiles[ i ].sSectorX, gMercProfiles[ i ].sSectorY, sTempString );
	...
	if( gpActiveListBox->sCurSelectedItem >= 0 )
	{
		...
		if( gfUseLocalNPCs )
		{
			...
			GetDebugLocationString( gubCurrentNpcInSector[ gpActiveListBox->sCurSelectedItem ], sTempString, 64 );
...
//			GetShortSectorString( gMercProfiles[ gubCurrentNpcInSector[ gpActiveListBox->sCurSelectedItem ] ].sSectorX, gMercProfiles[ gubCurrentNpcInSector[ gpActiveListBox->sCurSelectedItem ] ].sSectorY, sTempString );
		...
		else
		{
			...
			GetDebugLocationString( gpActiveListBox->sCurSelectedItem, sTempString, 64 );
//			GetShortSectorString( gMercProfiles[ gpActiveListBox->sCurSelectedItem ].sSectorX, gMercProfiles[ gpActiveListBox->sCurSelectedItem ].sSectorY, sTempString );
		...
		if( gfUseLocalNPCs )
			WSTR_SPrintf( zButtonName, 256, L"%d - %ls", gubCurrentNpcInSector[ gpActiveListBox->sCurSelectedItem ],  gMercProfiles[ gubCurrentNpcInSector[ gpActiveListBox->sCurSelectedItem ] ].zNickname );
		else
			WSTR_SPrintf( zButtonName, 256, L"%d - %ls", gpActiveListBox->sCurSelectedItem,  gMercProfiles[ gpActiveListBox->sCurSelectedItem ].zNickname );
...
//			Assert(0);
		...
		WSTR_SPrintf( zButtonName, 256, L"%d - %ls", gpActiveListBox->sCurSelectedItem, zItemName );
...
void BtnQuestDebugAddNpcToLocationButtonCallback(GUI_BUTTON *btn,INT32 reason)
{
	...
	if(reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		WSTR_SPrintf( zTemp, 512, L"%ls where %ls will be added.", QuestDebugText[ QUEST_DBS_ENTER_GRID_NUM ], gMercProfiles[ gNpcListBox.sCurSelectedItem ].zNickname );
...
//			Assert(0);
		...
		WSTR_SPrintf( zTemp, 512, L"%ls where the %ls will be added.", QuestDebugText[ QUEST_DBS_ENTER_GRID_NUM ], zItemName );
...
void BtnQuestDebugChangeDayButtonCallback(GUI_BUTTON *btn,INT32 reason)
{
	...
	if(reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		WSTR_SPrintf( zTemp, 512, L"%ls   Current Day is %d", QuestDebugText[ QUEST_DBS_PLEASE_ENTER_DAY ], GetWorldDay() );
...
void BtnQuestDebugNPCRefreshButtonButtonCallback(GUI_BUTTON *btn,INT32 reason)
{
	...
	if(reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		...
		if( fRetVal )
		{
			WSTR_SPrintf( zTemp, 128, L"%ls %ls", QuestDebugText[ QUEST_DBS_REFRESH_OK ], gMercProfiles[ ubMercID ].zNickname );
		}
		else
		{
			WSTR_SPrintf( zTemp, 128, L"%ls %ls", QuestDebugText[ QUEST_DBS_REFRESH_FAILED ], gMercProfiles[ ubMercID ].zNickname );
		}
...
//qqq
		WSTR_SPrintf( String, 512, L"%ls %ls %d \"%ls\" ( %ls )", QuestDebugText[ QUEST_DBS_ENTER_NEW_VALUE ], QuestDebugText[ QUEST_DBS_QUEST_NUM ], gubCurQuestSelected, QuestDescText[ gubCurQuestSelected ], QuestDebugText[ QUEST_DBS_0_1_2 ] );
...
{ 
	...
	else if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP)
	{
		...
		if( FactDescText[ gusCurFactSelected ][0] == '\0' )
			WSTR_SPrintf( String, 512, L"%ls %ls %d \"%ls\" ( %ls )", QuestDebugText[ QUEST_DBS_ENTER_NEW_VALUE ], QuestDebugText[ QUEST_DBS_FACT_NUM ], gusCurFactSelected, QuestDebugText[ QUEST_DBS_NO_TEXT ], QuestDebugText[ QUEST_DBS_0_1 ] );
		else
			WSTR_SPrintf( String, 512, L"%ls %ls %d \"%ls\" ( %ls )", QuestDebugText[ QUEST_DBS_ENTER_NEW_VALUE ], QuestDebugText[ QUEST_DBS_FACT_NUM ], gusCurFactSelected, FactDescText[ gusCurFactSelected ], QuestDebugText[ QUEST_DBS_0_1 ] );
...
*/
	...
	WSTR_SPrintf( sTemp, 640, L"%d", gsQdsEnteringGridNo );
...
void StartMercTalkingFromQuoteNum( INT32 iQuoteToStartTalkingFrom )
{
	...
	{
		WSTR_SPrintf( zTemp, 512, L"Please enter a value between 0 and %d", uiMaxNumberOfQuotes );
		DoQDSMessageBox( MSG_BOX_BASIC_STYLE, zTemp, QUEST_DEBUG_SCREEN, MSG_BOX_FLAG_OK, NULL );
	}
	else
	{
...
void DisplayQDSCurrentlyQuoteNum( )
{
	...
	WSTR_SPrintf( zTemp, 512, L"'%ls' is currently saying quote #%d", gMercProfiles[ gTalkingMercSoldier->ubProfile ].zNickname, giSelectedMercCurrentQuote-1 );
...
void GetDebugLocationString( UINT16 usProfileID, STR16 pzText, UINT16 usMaxLen )
{
	...
	if( pSoldier != NULL && pSoldier->bActive && pSoldier->uiStatusFlags & SOLDIER_OFF_MAP )
	{
		...
		WSTR_SPrintf( pzText, usMaxLen, L"On Schdl.");
	}

	//if the soldier is dead
	else if( gMercProfiles[ usProfileID ].bMercStatus == MERC_IS_DEAD )
	{
		WSTR_SPrintf( pzText, usMaxLen, L"Dead");
	}

	//the soldier is in this sector
	else if( pSoldier != NULL )
	{
		GetShortSectorString( pSoldier->sSectorX, pSoldier->sSectorY, pzText, usMaxLen );
	}

	//else the soldier is in a different map
	else
	{
		GetShortSectorString( gMercProfiles[ usProfileID ].sSectorX, gMercProfiles[ usProfileID ].sSectorY, pzText, usMaxLen );
	}
}
