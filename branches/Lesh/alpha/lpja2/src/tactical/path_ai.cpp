...
INT32 FindBestPath(SOLDIERTYPE *s , INT16 sDestination, INT8 ubLevel, INT16 usMovementMode, INT8 bCopy, UINT8 fFlags )
{
  ...
  UINT16    usMovementModeToUseForAPs;
	...
	// Lesh: try avoid lights during battle
	fAvoidLight = (s->bTeam != CREATURE_TEAM) && (s->bAlertStatus != STATUS_GREEN) && NightTime();
...
/*
	...
	*/
			...
			if ( fAvoidLight )
			{
				ubShadeLevel = gpWorldLevelData[ newLoc ].pLandHead->ubShadeLevel;
				//ubShadeLevel = (ubShadeLevel < 0) ? (0):(ubShadeLevel);
				ubShadeLevel = MAX_SHADE_LEVEL - ubShadeLevel;
				nextCost += ubShadeLevel * TRAVELCOST_LIGHT;
			}
