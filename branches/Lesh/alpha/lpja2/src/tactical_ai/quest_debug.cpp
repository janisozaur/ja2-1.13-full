...
void ToggleQuestDebugModes( UINT8 ubType )
{
	...
	if( ubLevel == QD_OUTPUT_NONE )
		ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION, L"%ls No Output", sType );
	else if( ubLevel == QD_OUTPUT_LEVEL_ALL )
		ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION, L"%ls All messages", sType );
	else
		ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION, L"%ls Level %d", sType, ubLevel );
}
