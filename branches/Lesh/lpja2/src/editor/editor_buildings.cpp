// WANNE: EDITOR: done
	...
	#include "render_world.h"	//ClearRenderFlags();
	#include "world_man.h"			//HideStructOfGivenType()
...
void ExtractAndUpdateDoorInfo()
{
	...
	num = SGP_min( GetNumericStrictValueFromField( 0 ), NUM_LOCKS-1 );
	door.ubLockID = (UINT8)num;
	SetInputFieldStringWithNumericStrictValue( 0, num );
	if( num >= 0 )
		fCursor = TRUE;

	num = SGP_min( SGP_max( GetNumericStrictValueFromField( 1 ), 0 ), 10 );
	door.ubTrapID = (UINT8)num;
	SetInputFieldStringWithNumericStrictValue( 1, num );
	if( num ) 
		fCursor = TRUE;

	num = SGP_min( SGP_max( GetNumericStrictValueFromField( 2 ), 0 ), 20 );
