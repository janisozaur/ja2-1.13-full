...
*/
	...
	//Create a string for the weight ( %s ) ( where %s is the weight string, either kg or lbs )
	WSTR_SPrintf( sTemp, 128, BobbyROrderFormText[BOBBYR_COST], GetWeightUnitString( ) );
		...
		if( guiSubTotal && gfCanAcceptOrder )
		{
			...
			else
			{
				...
				else
				{
					...
					WSTR_SPrintf( zTemp, 128, BobbyROrderFormText[BOBBYR_CONFIRM_DEST],  *BobbyROrderLocations[gbSelectedCity].psCityLoc );
...
void DisplayPurchasedItems( BOOLEAN fCalledFromOrderPage, UINT16 usGridX, UINT16 usGridY, BobbyRayPurchaseStruct *pBobbyRayPurchase, BOOLEAN fJustDisplayTitles, INT32 iOrderNum )
{
	...
	//Create a string for the weight ( %s ) ( where %s is the weight string, either kg or lbs )
	WSTR_SPrintf( sTemp, 20, BobbyROrderFormText[BOBBYR_WEIGHT], GetWeightUnitString( ) );
...
//		if( BobbyRayPurchases[ i ].ubNumberPurchased )
		if( pBobbyRayPurchase[i].ubNumberPurchased )	
		{
			...
			//qty
			WSTR_SPrintf(sTemp, 20, L"%3d", pBobbyRayPurchase[i].ubNumberPurchased );
			...
			//weight
			WSTR_SPrintf( sTemp, 20, L"%3.1f", GetWeightBasedOnMetricOption( Item[ pBobbyRayPurchase[i].usItemIndex ].ubWeight ) / (FLOAT)( 10.0 ) * pBobbyRayPurchase[i].ubNumberPurchased );
...
//				LoadEncryptedDataFromFile(BOBBYRDESCFILE, sText, uiStartLoc, BOBBYR_ITEM_DESC_NAME_SIZE);
			...
			//unit price
			WSTR_SPrintf(sTemp, 20, L"%d", CalcBobbyRayCost( pBobbyRayPurchase[i].usItemIndex, pBobbyRayPurchase[i].usBobbyItemIndex, pBobbyRayPurchase[i].fUsed ));
			...
			WSTR_SPrintf(sTemp, 20, L"%d", uiTotal );
			InsertCommasForDollarFigure( sTemp );
			InsertDollarSignInToString( sTemp );
...
//if there is a shipment, display the s&h charge
	if( iSubTotal )
	{
		...
		WSTR_SPrintf(sTemp, 20, L"%d", iSubTotal );
		InsertCommasForDollarFigure( sTemp );
		InsertDollarSignInToString( sTemp );
		...
		WSTR_SPrintf(sTemp, 20, L"%d", iShippingCost );
		InsertCommasForDollarFigure( sTemp );
		InsertDollarSignInToString( sTemp );
		...
		WSTR_SPrintf(sTemp, 20, L"%d", giGrandTotal );
		InsertCommasForDollarFigure( sTemp );
		InsertDollarSignInToString( sTemp );
...
void DisplayShippingLocationCity()
{
	...
	if( gbSelectedCity != -1 )
	{
		WSTR_SPrintf( sTemp, 40, L"%d", ( INT32 )(BobbyROrderLocations[gbSelectedCity].usOverNightExpress/GetWeightBasedOnMetricOption( 1 ) )  );
		InsertCommasForDollarFigure( sTemp );
		InsertDollarSignInToString( sTemp );
	}
	...
	if( gbSelectedCity != -1 )
	{
		WSTR_SPrintf( sTemp, 40, L"%d", ( INT32 )( BobbyROrderLocations[gbSelectedCity].us2DaysService / GetWeightBasedOnMetricOption( 1 ) ) );
		InsertCommasForDollarFigure( sTemp );
		InsertDollarSignInToString( sTemp );
	}
	...
	if( gbSelectedCity != -1 )
	{
		WSTR_SPrintf( sTemp, 40, L"%d", (INT32 )( BobbyROrderLocations[gbSelectedCity].usStandardService / GetWeightBasedOnMetricOption( 1 ) ) );
		InsertCommasForDollarFigure( sTemp );
		InsertDollarSignInToString( sTemp );
	}
