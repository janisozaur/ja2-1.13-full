...
void EnterLaptopInitInsuranceContract()
{
	...
	WSTR_SPrintf( zTextField, 14, L"%d", 0 );
...
BOOLEAN DisplayOrderGrid( UINT8 ubGridNumber, UINT8 ubMercID )
{
  ...
  HVOBJECT hPixHandle;
	...
	WSTR_SPrintf( sText, 800, L"%d", pSoldier->iTotalContractLength ); 
	...
	else
	{
		...
		WSTR_SPrintf( sText, 800, L"%d", iCostOfContract ); 
	...
	//
	WSTR_SPrintf( sText, 800, L"%d", CalculateSoldiersInsuranceContractLength( pSoldier ) );
