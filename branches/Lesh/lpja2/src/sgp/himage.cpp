...
HIMAGE CreateImage( SGPFILENAME ImageFile, UINT16 fContents )
{
	...
	{
		fprintf(stderr, "IMAGE: Resource file %s does not exist\n", ImageFile);
	...
	{
		fprintf(stderr, "IMAGE: Failed to load file %s\n", hImage->ImageFile);
		return( NULL );
	}
...
BOOLEAN LoadImageData( HIMAGE hImage, UINT16 fContents )
{
	...
	switch( hImage->iFileLoader )
	{
		...
		default:
			fprintf(stderr, "IMAGE: Unknown image loader was specified (%s)\n", hImage->ImageFile);
