...
BOOLEAN LoadSTCIFileToImage( HIMAGE hImage, UINT16 fContents )
{
	...
	{
		fprintf(stderr, "STCI: Problem reading STCI header (%s), bytes = %d/%d, header_id = %c%c%c%c\n",
			TempImage.ImageFile, uiBytesRead, STCI_HEADER_SIZE,
			Header.cID[0], Header.cID[1], Header.cID[2], Header.cID[3] );
	...
	if (Header.fFlags & STCI_RGB)
	{
		...
		{
			fprintf(stderr, "STCI: Problem reading RGB image (%s)\n", TempImage.ImageFile);
	...
	else if (Header.fFlags & STCI_INDEXED)
	{
		...
		{
			fprintf(stderr, "STCI: Problem reading palettized image (%s)\n", TempImage.ImageFile);
