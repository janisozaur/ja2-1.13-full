...
void PopupMovementMenu( UI_EVENT *pUIEvent )
{
  ...
  iActionIcons[ WALK_ICON ] = QuickCreateButton( iIconImages[ WALK_IMAGES ], (INT16)(iMenuAnchorX + 40 ), (INT16)(iMenuAnchorY  ),
	...
	if ( pSoldier->uiStatusFlags & SOLDIER_VEHICLE )
	{
		...
		WSTR_SPrintf( zActionString, 50, TacticalStr[ NOT_APPLICABLE_POPUPTEXT ] );
		fDisableAction = TRUE;
	}
	else
	{
		...
		else
		{
			...
			switch( Item[ pSoldier->inv[ HANDPOS ].usItem ].usItemClass )
			{
				case IC_PUNCH:
					...
					WSTR_SPrintf( zActionString, 50, TacticalStr[ USE_HANDTOHAND_POPUPTEXT ] );
					break;

				case IC_GUN:

					uiActionImages = TARGETACTIONC_IMAGES;
					WSTR_SPrintf( zActionString, 50, TacticalStr[ USE_FIREARM_POPUPTEXT ] );
					break;

				case IC_BLADE:

					uiActionImages = KNIFEACTIONC_IMAGES;
					WSTR_SPrintf( zActionString, 50, TacticalStr[ USE_BLADE_POPUPTEXT ] );
					break;
				...
				case IC_BOMB:
					...
					WSTR_SPrintf( zActionString, 50, TacticalStr[ USE_EXPLOSIVE_POPUPTEXT ] );
					break;

				case IC_MEDKIT:

					uiActionImages = AIDACTIONC_IMAGES;
					WSTR_SPrintf( zActionString, 50, TacticalStr[ USE_MEDKIT_POPUPTEXT ] );
					break;
...
void DrawSelectedUIAboveGuy( UINT16 usSoldierID )
{
	...
	if ( pSoldier->ubProfile != NO_PROFILE || ( pSoldier->uiStatusFlags & SOLDIER_VEHICLE ) )
	{
		...
		{
			WSTR_SPrintf( NameStr, 50, TacticalStr[ CATCH_STR ] );
      ...
      fRaiseName = TRUE;
		...
		{
			WSTR_SPrintf( NameStr, 50, TacticalStr[ RELOAD_STR ] );
			FindFontCenterCoordinates( sXPos, (INT16)(sYPos ), (INT16)(80 ), 1, NameStr, TINYFONT1, &sX, &sY );
			gprintfdirty( sX, sY, NameStr );
			mprintf( sX, sY, NameStr );
      fRaiseName = TRUE;
		...
		{
			WSTR_SPrintf( NameStr, 50, pMessageStrings[ MSG_PASS ] );
			FindFontCenterCoordinates( sXPos, (INT16)(sYPos ), (INT16)(80 ), 1, NameStr, TINYFONT1, &sX, &sY );
			gprintfdirty( sX, sY, NameStr );
			mprintf( sX, sY, NameStr );
      fRaiseName = TRUE;
		...
		else if ( pSoldier->bAssignment >= ON_DUTY )
		{
				...
				WSTR_SPrintf( NameStr, 50, L"(%ls)", pAssignmentStrings[ pSoldier->bAssignment ] );
				FindFontCenterCoordinates( sXPos, (INT16)(sYPos ), (INT16)(80 ), 1, NameStr, TINYFONT1, &sX, &sY );
				gprintfdirty( sX, sY, NameStr );
				mprintf( sX, sY, NameStr );
		fRaiseName = TRUE;
		...
		{
				WSTR_SPrintf( NameStr, 50, gzLateLocalizedString[ 34 ], ( pSoldier->bAssignment + 1 ) );
				FindFontCenterCoordinates( sXPos, (INT16)(sYPos ), (INT16)(80 ), 1, NameStr, TINYFONT1, &sX, &sY );
				gprintfdirty( sX, sY, NameStr );
				mprintf( sX, sY, NameStr );
		fRaiseName = TRUE;
		...
		if ( fDoName )
		{
		  if ( fRaiseName )
		  {
			  WSTR_SPrintf( NameStr, 50, L"%ls", pSoldier->name );
		...
		{    // RPC_RECRUITED( pSoldier ) || AM_AN_EPC( pSoldier ) || ( pSoldier->uiStatusFlags & SOLDIER_VEHICLE ) )
			if ( gfUIMouseOnValidCatcher == 2 && pSoldier->ubID == gubUIValidCatcherID )
			{
				...
				WSTR_SPrintf( NameStr, 50, TacticalStr[ GIVE_STR ] );
				FindFontCenterCoordinates( sXPos, (INT16)(sYPos + 10 ), (INT16)(80 ), 1, NameStr, TINYFONT1, &sX, &sY );
				gprintfdirty( sX, sY, NameStr );
				mprintf( sX, sY, NameStr );
	...
	else //pSoldier->ubProfile != NO_PROFILE || ( pSoldier->uiStatusFlags & SOLDIER_VEHICLE )
	{
		if ( pSoldier->bLevel != 0 )
		{
			...
			SetFontForeground( FONT_YELLOW );
				...
				WSTR_SPrintf( NameStr, 50, gzLateLocalizedString[ 15 ] );
				FindFontCenterCoordinates( sXPos, (INT16)(sYPos + 10 ), (INT16)(80 ), 1, NameStr, TINYFONT1, &sX, &sY );
				gprintfdirty( sX, sY, NameStr );
				mprintf( sX, sY, NameStr );
		}
...
void BeginOverlayMessage( UINT32 uiFont, wchar_t *pFontString, ... )
{
	...
	WSTR_VSPrintf(SlideString, 512, pFontString, argptr);	// process gprintf string (get output str)
	va_end(argptr);
...
void PopupDoorOpenMenu( BOOLEAN fClosingDoor )
{
	...
	{
		WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ USE_KEYRING_ICON ] );
	}
	else
	{
		WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ USE_KEYRING_ICON ], AP_UNLOCK_DOOR );
	}
	...
	if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
	{
		WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ USE_CROWBAR_ICON ] );
	}
	else
	{
		WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ USE_CROWBAR_ICON ], AP_USE_CROWBAR );
	}
	...
	if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
	{
		WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ LOCKPICK_DOOR_ICON ] );
	}
	else
	{
		WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ LOCKPICK_DOOR_ICON ], AP_PICKLOCK );
	}
	...
	if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
	{
		WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ EXPLOSIVE_DOOR_ICON ] );
	}
	else
	{
		WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ EXPLOSIVE_DOOR_ICON ], AP_EXPLODE_DOOR );
	}
	...
	if ( fClosingDoor )
	{
		if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
		{
			WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ CANCEL_ICON + 1 ] );
		}
		else
		{
			WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ CANCEL_ICON + 1 ], AP_OPEN_DOOR );
		}
	}
	else
	{
		if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
		{
			WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ OPEN_DOOR_ICON ] );
		}
		else
		{
			WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ OPEN_DOOR_ICON ], AP_OPEN_DOOR );
		}
	}
	...
	if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
	{
		WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ EXAMINE_DOOR_ICON ] );
	}
	else
	{
		WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ EXAMINE_DOOR_ICON ], AP_EXAMINE_DOOR );
	}
	...
	if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
	{
		WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ BOOT_DOOR_ICON ] );
	}
	else
	{
		WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ BOOT_DOOR_ICON ], AP_BOOT_DOOR );
	}
	...
	if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT ) )
	{
		WSTR_SPrintf( zDisp, 100, pTacticalPopupButtonStrings[ UNTRAP_DOOR_ICON ] );
	}
	else
	{
		WSTR_SPrintf( zDisp, 100, L"%ls ( %d )", pTacticalPopupButtonStrings[ UNTRAP_DOOR_ICON ], AP_UNTRAP_DOOR );
	}
...
void InternalBeginUIMessage( BOOLEAN fUseSkullIcon, wchar_t *pFontString, ... )
{
	...
	va_start(argptr, pFontString);       	// Set up variable argument pointer
	WSTR_VSPrintf(MsgString, 512, pFontString, argptr);	// process gprintf string (get output str)
	va_end(argptr);
