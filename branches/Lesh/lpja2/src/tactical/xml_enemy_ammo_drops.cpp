...
{	
	...
	if(pData->currentDepth <= pData->maxReadDepth) //we're at the end of an element that we've been reading
	{
		...
		{
			pData->curElement = ELEMENT;
			pData->curAmmoDrop.uiIndex   = (UINT32) atoi(pData->szCharData);
		}
		else if(strcmp(name, "uiType") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curAmmoDrop.uiType   = (UINT32) atoi(pData->szCharData);
		}
		else if(strcmp(name, "ubEnemyDropRate") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curAmmoDrop.ubEnemyDropRate  = (UINT8) atoi(pData->szCharData);
		}
		else if(strcmp(name, "ubMilitiaDropRate") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curAmmoDrop.ubMilitiaDropRate  = (UINT8) atoi(pData->szCharData);
		}
