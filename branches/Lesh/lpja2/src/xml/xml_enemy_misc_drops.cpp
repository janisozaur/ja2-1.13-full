...
{	
	...
	if(pData->currentDepth <= pData->maxReadDepth) //we're at the end of an element that we've been reading
	{
		...
		{
			pData->curElement = ELEMENT;
			pData->curMiscDrop.uiIndex   = (UINT32) atoi(pData->szCharData);
		}
		else if(strcmp(name, "usItemClass") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curMiscDrop.usItemClass   = (UINT32) atoi(pData->szCharData);
		}
		else if(strcmp(name, "ubEnemyDropRate") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curMiscDrop.ubEnemyDropRate  = (UINT8) atoi(pData->szCharData);
		}
		else if(strcmp(name, "ubMilitiaDropRate") == 0)
		{
			pData->curElement = ELEMENT;
			pData->curMiscDrop.ubMilitiaDropRate  = (UINT8) atoi(pData->szCharData);
		}
