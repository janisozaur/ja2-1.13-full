	...
	if(pData->currentDepth <= pData->maxReadDepth) //are we reading this element?
	{
		...
		else if(strcmp(name, "ITEM") == 0 && pData->curElement == ELEMENT_LIST)
		{
			...
			pData->maxReadDepth++; //we are not skipping this element
				...
				strcmp(name, "DefaultAttachment2") == 0 ||
				strcmp(name, "DefaultAttachment3") == 0 ||
				strcmp(name, "DefaultAttachment4") == 0 ||
...
itemEndElementHandle(void *userData, const char *name)
{
	...
	if(pData->currentDepth <= pData->maxReadDepth) //we're at the end of an element that we've been reading
	{
		...
		else if(strcmp(name, "DefaultAttachment2")	 == 0)
		{
			pData->curElement = ELEMENT;
			pData->curItem.defaultattachment[1]  = (UINT16) atol(pData->szCharData);
		}
		else if(strcmp(name, "DefaultAttachment3")	 == 0)
		{
			pData->curElement = ELEMENT;
			pData->curItem.defaultattachment[2]  = (UINT16) atol(pData->szCharData);
		}
		else if(strcmp(name, "DefaultAttachment4")	 == 0)
		{
			pData->curElement = ELEMENT;
			pData->curItem.defaultattachment[3]  = (UINT16) atol(pData->szCharData);
		}
...
//#define		BOBBYR_ITEM_DESC_FILE_SIZE				800
			...
			FilePrintf(hFile,"\t\t<DefaultAttachment>%d</DefaultAttachment>\r\n",						Item[cnt].defaultattachment[0] );
			FilePrintf(hFile,"\t\t<DefaultAttachment2>%d</DefaultAttachment2>\r\n",						Item[cnt].defaultattachment[1] );
			FilePrintf(hFile,"\t\t<DefaultAttachment3>%d</DefaultAttachment3>\r\n",						Item[cnt].defaultattachment[2] );
			FilePrintf(hFile,"\t\t<DefaultAttachment4>%d</DefaultAttachment4>\r\n",						Item[cnt].defaultattachment[3] );
